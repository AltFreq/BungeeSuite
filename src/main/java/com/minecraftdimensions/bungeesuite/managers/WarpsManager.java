...
public class WarpsManager {
	...
	public static void setWarp(BSPlayer sender, String name, Location loc, boolean hidden, boolean global) throws SQLException{
		...
		if(doesWarpExist(name)){
			...
			w.setGlobal(global);
			w.setHidden(hidden);
			SQLManager.standardQuery("UPDATE BungeeWarps SET server='" + loc.getServer().getName()
					+ "', world='" + loc.getWorld() + "', x=" + loc.getX() + ", y=" + loc.getY() + ", z="
					+ loc.getZ() + ", yaw=" + loc.getYaw() + ", pitch= " + loc.getPitch()
					+ ", hidden="+hidden+", global = "+global+" WHERE warpname='" + name + "'");
	...
	public static void sendPlayerToWarp(String sender, String player,
			String warp, boolean permission, boolean silent) {
		BSPlayer s = PlayerManager.getPlayer(sender);
		...
		if(!w.isGlobal() &&! w.isHidden()){
			if(!w.getLocation().getServer().equals(p.getServer().getInfo())){
