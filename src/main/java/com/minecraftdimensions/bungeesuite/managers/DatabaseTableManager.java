...
public class DatabaseTableManager {
	...
	public static void insertData(String table, String column, String data)
			throws SQLException {
		SQLManager.standardQuery("INSERT INTO " + table + " (" + column + ") VALUE ("
				+ data + ");");
	}
	...
	public static void createDefaultTables(){
		...
		//BungeeBans
		runTableQuery("BungeeBans", "CREATE TABLE BungeeBans (player VARCHAR(100), banned_by VARCHAR(100), reason VARCHAR(255), type VARCHAR(100), banned_on DATETIME NOT NULL, banned_until DATETIME, CONSTRAINT pk_bannedPlayer PRIMARY KEY (player), CONSTRAINT fk_playerkey FOREIGN KEY (player) REFERENCES BungeePlayers (playername) ON UPDATE CASCADE ON DELETE CASCADE, CONSTRAINT fk_banner FOREIGN KEY (banned_by) REFERENCES BungeePlayers (playername) ON UPDATE CASCADE ON DELETE CASCADE)");
		//BungeeChatIgnores
